...
> As of Harper v4.6, the `start/startOnMainMethod` and `handle/setup|File/Directory` extension API has been marked as **legacy**. The legacy extension API is still supported, but will likely be removed in a future major release. It has been replaced by the new, more extensible [Plugin API](./plugins.md).
