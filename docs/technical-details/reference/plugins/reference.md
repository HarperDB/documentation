...
A plugin must specify a `config.yaml` file with a singular `extensionModule` option. This must be a path to the module source code. The path must resolve from the root of the plugin module directory.

For example, the [Harper Next.js Plugin](https://github.com/HarperDB/nextjs) `config.yaml` specifies `extensionModule: ./extension.js`.
...
It is also recommended that all plugins have a `package.json` that specifies JavaScript package metadata such as name, version, type, etc. Since plugins are just JavaScript packages, they can do anything a JavaScript package can normally do. It can be written in TypeScript, and compiled to JavaScript. It can export an executable (using the [bin](https://docs.npmjs.com/cli/configuring-npm/package-json#bin) property). It can be published to npm. The possibilities are endless!

The key to a plugin module is the named export `handleComponent()`. This is the only method that a plugin module must export. It is the entry point for the plugin, and is executed by Harper when the plugin is used by an application.

> An plugin cannot export both `handleComponent()` and any of the [legacy extension](../legacy-extensions.md) methods. Harper will throw an error if both are defined.
...
> This is a simplified form of the [static](./built-in.md#static) built-in plugin.
...
This is the only method a plugin module must export. It can be async and is awaited by the component loader. The `scope` argument provides access to the component's configuration, resources, and other APIs.
